...
public class FutureMonitoring implements Runnable {
    ...
    private static final Map<UniqueID, String> nodeUrls = new ConcurrentHashMap<UniqueID, String>();

    static final Logger logger = ProActiveLogger.getLogger(Loggers.CORE);
    static {

        /* Dynamically configurable to make shorter tests */
        final String ttm = CentralPAPropertyRepository.PA_FUTUREMONITORING_TTM.getValueAsString();
        if (ttm != null) {
            final int tmp = Integer.parseInt(ttm);
            if (tmp >= 0) {
                TTM = tmp;
            } else {
        ...
        if (TTM > 0) {
            final Thread t = new Thread(new FutureMonitoring(), "Monitoring the Futures");
            t.setDaemon(true);
            t.start();
        } else {
     ...
     */
    private static boolean pingBody(final UniqueID bodyId) {
        boolean pinged = false;
        FutureMonitoringPingFailureException bodyException = null;
        final Collection<FutureProxy> futures = futuresToMonitor.get(bodyId);
        final String nodeUrl = nodeUrls.get(bodyId);
        if (futures == null) {
        ...
        for (final FutureProxy fp : futures) {
            if (!pinged) {
                ...
                if (body != null) {
                    ...
                    pinged = true;
                    try {
                        final Integer state = (Integer) body.receiveFTMessage(HEARTBEAT_MSG);
    ...
    @Override
        ...
        for (;;) {
            ...
            for (final UniqueID bodyId : futuresToMonitor.keySet()) {
                final boolean pingedThisBody = pingBody(bodyId);
                if (pingedThisBody) {
                    pingedOneBody = true;
                    monitoringDelay();
                }
    ...
    private static UniqueID getUpdaterBodyId(final FutureProxy fp) {
        if (isFTEnabled()) {
            return null;
        }
        final UniversalBody body = fp.getUpdater();
        if (body == null) {
    ...
    private static Pair<UniqueID, String> getUpdaterBodyIdAndNodeUrl(final FutureProxy fp) {
        if (isFTEnabled()) {
            return null;
        }
        final UniversalBody body = fp.getUpdater();
    ...
    public static void removeFuture(final FutureProxy fp) {
        final UniqueID updaterId = getUpdaterBodyId(fp);
        if (updaterId == null) {
            return;
        }
        synchronized (futuresToMonitor) {
             ...
             */
            final ConcurrentLinkedQueue<FutureProxy> futures = futuresToMonitor.get(updaterId);
            if (futures != null) {
                futures.remove(fp);
                if (futures.isEmpty()) {
    ...
    public static void monitorFutureProxy(final FutureProxy fp) {
        if (fp.isAvailable()) {
            return;
        }
        final Pair<UniqueID, String> pair = getUpdaterBodyIdAndNodeUrl(fp);
        final UniqueID updaterId = pair.getFirst();
        if (updaterId == null) {
            return;
        }
        final String nodeUrl = pair.getSecond();
        synchronized (futuresToMonitor) {
            /*
    ...
    private static boolean isFTEnabled() {
        if (!FTEnabled) {
            final Collection<LocalNode> nodes = ProActiveRuntimeImpl.getProActiveRuntime().getLocalNodes();
            if (nodes.size() != lastNumberOfNodes) {
                lastNumberOfNodes = nodes.size();
                for (final LocalNode node : nodes) {
